...
# Récupère l’XP théorique en fonction de la fréquence, répétition et intensité
func get_xp_theorique(freq: String, reps: int, intensity: int) -> int:
	if quest_table.has(freq) and quest_table[freq].has(reps):
		var values: Array = quest_table[freq][reps]["xp"]
		if intensity >= 1 and intensity <= values.size():
			return values[intensity - 1]
	return 0

# Récupère le malus pour une fréquence/répétition
func get_malus(freq: String, reps: int) -> float:
	if quest_table.has(freq) and quest_table[freq].has(reps):
		return quest_table[freq][reps]["malus"]
	return 0.0

# Calcule l’XP net (après malus)
func get_xp_net(freq: String, reps: int, intensity: int, fails: int) -> int:
	var base_xp: int = get_xp_theorique(freq, reps, intensity)
